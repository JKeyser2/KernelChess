...
int main(){
    ...
    while(checkmate == false){
        ...
        printf("iteration: %d \n", theIteration);
        printf("White Move: \n");
        checkResult = WhiteCpuPickMove(theBoardArray);
        if(checkResult == 3){
            checkmate = true;
        }
        isThereStillKing = LookForBlackKing(theBoardArray);
        if(isThereStillKing == -1){
            printf("King no more\n");
            checkmate = true;
        }
        PrintBoard(theBoardArray);
        printf("Black Move: \n");
        checkResult = BlackCpuPickMove(theBoardArray);
        if(checkResult == 3){
            checkmate = true;
        }
        isThereStillKing = LookForWhiteKing(theBoardArray);
        if(isThereStillKing == -1){
            printf("King no more\n");
            checkmate = true;
        }
    ...
    //FindCheckAndCheckmateOnWhiteKing(theBoardArray);
    //FindCheckAndCheckmateOnBlackKing(theBoardArray);
        ...
        if(moveIsPossible == true){
            ...
            if(pieceEndLocation[0][0] == 0 && strcmp(upgradedPiece, "WR") == 0){
                ...
                if(checkedSelf == 2 || checkedSelf == 3){
                    ...
                    printf("Can't make that move, puts your king in check/checkmate\n");
                    return -1;
                }else{
                    printf("Move made\n");
